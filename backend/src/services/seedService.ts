...
export async function seedDatabase() {
  ...
  const user8  = await prisma.user.create({ data: { name: 'Priya Nair',       email: 'priya@lab.com',   password: hash, isAdmin: false } });
  const user9  = await prisma.user.create({ data: { name: 'Lucas Müller',     email: 'lucas@lab.com',   password: hash, isAdmin: false } });
  const user10 = await prisma.user.create({ data: { name: 'Aisha Okonkwo',    email: 'aisha@lab.com',   password: hash, isAdmin: false } });
  const user11 = await prisma.user.create({ data: { name: 'Tom Nguyen',       email: 'tom@lab.com',     password: hash, isAdmin: false } });
  const user12 = await prisma.user.create({ data: { name: 'Sofia Petrov',     email: 'sofia@lab.com',   password: hash, isAdmin: false } });

  console.log('Created 13 users (2 admins, 11 regular)');
  ...
  console.log('');
  console.log('Summary:');
  console.log('   Users:   13  (2 admins, 11 regular)');
  ...
  console.log('   priya@lab.com   lucas@lab.com   aisha@lab.com');
